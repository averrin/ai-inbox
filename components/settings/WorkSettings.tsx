...
import { View, Text, Switch, FlatList, ActivityIndicator, TouchableOpacity, Modal, TextInput } from 'react-native';
...
export function WorkSettings() {
    ...
    // Auto-repair: If workAccountId is missing but we have selected work calendars, try to find an email
    useEffect(() => {
        if (!loading && calendars.length > 0 && workCalendarIds.length > 0 && !workAccountId) {
            const workCal = calendars.find(c => workCalendarIds.includes(c.id));
            if (workCal && workCal.source && workCal.source.name && workCal.source.name.includes('@')) {
                console.log('[WorkSettings] Auto-repairing workAccountId:', workCal.source.name);
                setWorkAccountId(workCal.source.name);
            }
        }
    }, [loading, calendars, workCalendarIds, workAccountId]);
    ...
    const toggleWorkCalendar = (id: string, sourceName?: string) => {
        let newIds;
        if (workCalendarIds.includes(id)) {
            newIds = workCalendarIds.filter(c => c !== id);
        } else {
            newIds = [...workCalendarIds, id];
            if (!visibleCalendarIds.includes(id)) {
                setVisibleCalendarIds([...visibleCalendarIds, id]);
            }
            // Auto-fill work account ID if empty and source looks like an email
            if (!workAccountId && sourceName && sourceName.includes('@')) {
                setWorkAccountId(sourceName);
            }
        }
        setWorkCalendarIds(newIds);
    };
    ...
    const renderItem = ({ item: cal }: { item: Calendar.Calendar }) => {
        const isWork = workCalendarIds.includes(cal.id);
