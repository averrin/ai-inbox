...
function CustomTabBar({ state, descriptors, navigation, navConfig, onOpenGroup, activeGroupId }: any) {
  ...
  const renderItem = (item: NavItemConfig) => {
    const isFocused = state.index === state.routes.findIndex((r: any) => r.name === item.id) || item.id === activeGroupId;
    ...
    const onPress = (event?: any) => {
      ...
      if (item.type === 'group') {
        // @ts-ignore - Event type inference is tricky here
        // Use pageX from event to center the menu
        // Default to screen center if event is missing (shouldn't happen with Touchable)
        const targetX = (event as any)?.nativeEvent?.pageX || 0;
        onOpenGroup(item, targetX);
      } else {
        ...
        >
          ...
          >
            <View style={{
              ...
              <Ionicons name={displayIcon as any} size={20} color={displayIconColor} />
            </View>
          </LinearGradient>
        </TouchableOpacity>
      );
...
function InnerTabNavigator({
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <GroupMenuOverlay
        visible={!!activeGroup}
        config={activeGroup?.config || null}
        targetX={activeGroup?.x || 0}
